...
- **Signing/notarization flag (for the DMG ticket).** Binaries are embedded in `postBuildScripts`, i.e. after the ad-hoc sign step. For a notarized release the embedded binaries must be built/copied **before** signing (e.g. a Copy Files phase with code-sign-on-copy, or a pre-sign script) so the bundle seal stays valid. Out of scope here (signing deferred), but the eventual DMG/Sparkle ticket must reorder this.
