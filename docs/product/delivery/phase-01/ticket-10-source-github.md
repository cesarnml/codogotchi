...
- **Rate-limit detection is signal-positive (post-AI-review).** A plain 403 with no rate-limit headers is NOT reported as `rateLimitHit: true`. Only 429, or 403 with `x-ratelimit-remaining: 0` (primary) or `retry-after` (secondary) qualify. This matters because token-scope errors and rate-limit errors have different remediation paths — collapsing them lets a misconfigured PAT silently masquerade as throttling. CodeRabbit flagged this on PR #11; patch keeps the secondary-rate-limit path explicit while excluding bare 403s.
- **Search and enrichment HTTP failures are swallowed (post-AI-review).** A thrown error from `http()` or `JSON.parse` returns the empty signal set rather than propagating. The CLI sync wrapper already runs each source in its own try/catch (per phase plan), but pushing graceful failure into the source itself means a transient blip on one PR enrichment skips that PR rather than aborting the rest. CodeRabbit flagged this on PR #11.
