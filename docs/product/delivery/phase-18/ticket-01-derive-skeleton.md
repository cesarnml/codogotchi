...
Deferred: selection state (`slotOccupants`, `prunedOrigins`, `userHiddenWindowKeys`, `windowSpawnedModes`, `combinedWindowIsMinimalist`, `evictedSessionFrames`) → P18.02. Push-spec state (`promptTimers`, `windowSessionIdentities`, `resolvedSessionTitles`, `activeConflictBubbleTargets`, conflict-bubble rate-limiter state, `lastAppliedIdleEscalationConfig`) → P18.03, per the title-resolution effect-seam decision (Grill-Me decision 3) which keeps title caching in the impure shell rather than `PoolMemory`. `DesiredWindows`/`DesiredWindow` carry the full eventual field set as placeholders (per plan decision 3) so P18.02/P18.03 extend rather than reshape; `derive` always returns an empty `DesiredWindows` this ticket. The Outcome's ".combined prompt-timer exemption" clause is also deferred with `promptTimers` itself to P18.03 — there is no prompt-timer state in `PoolMemory` yet for that exemption to apply to, so P18.01's eligibility-bounding block only prunes the three clock maps.
Contract note: none — ticket metadata (`Type: refactor`, `Scope: menubar`, `Red: required`) matched the work as scoped. Subagent adversarial review (codex-cli) found one actionable finding (the purity-gate test's substring match missed whitespace/attribute/declaration-specific `import AppKit` variants) — patched in `PoolDerivePurityGateTests.swift` with an anchored regex, verified against `import  AppKit`, `@_exported import AppKit`, and `import class AppKit.NSWindow` fixtures (see subagent-review commit). Its two Advisory Observations are folded into the Deferred line above (`resolvedSessionTitles`, `lastAppliedIdleEscalationConfig`, and the prompt-timer exemption wording) rather than a separate ledger entry, since both were pure Rationale-completeness gaps with no runtime behavior to fix.
