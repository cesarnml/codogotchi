...
Subagent-review finding (patched): `RenderKeyResolver.resolveRenderKeys` constructed `.origin(origin)` directly for the plain-origin fold branch (session-pets off, mode `.own`/other). For an origin literally equal to `"combined"`, this produced a `.origin("combined")` `WindowKey` distinct from `.combined` — a behavior change from the pre-refactor string convention, where a bare origin string `"combined"` was byte-identical to (and therefore collided with, and rendered as) the synthetic shared-combined-window key. Fixed by routing through `WindowKey(rawValue: origin) ?? .origin(origin)` instead of direct case construction, so the single rawValue parse path canonicalizes `"combined"` to `.combined` exactly as before. No real hook-registered platform origin (`claude_code`, `codex`, `cursor`, `vscode`, `antigravity`) is ever literally `"combined"`, so this was not reachable via legitimate hook-written `state.d/` data, but the fix preserves invariant 3 (no behavior change in key-resolution semantics) byte-for-byte regardless. Added `testPlainOriginLiterallyNamedCombinedFoldsToCombinedKey` regression coverage in `PerSessionReaderTests.swift`.

Advisory doc-drift correction: line 12's kill-list claim ("hits only `WindowKey`'s own parse/serialize, persistence encode/decode boundaries, and test fixtures") is not literally accurate — a plain `grep -rn '"combined"' apps/menubar/Sources` also hits `FloatingPetWindowPool.swift` for the separate assignment-slot and platform-attribution "combined" concepts named in the implementation note above (`AssignmentsJsonReader.resolve(origin:)`, `PlatformAttribution.init?(origin:)`). Those are deliberately out of scope (different concepts sharing the literal string, verified via codebase trace), not policy-site violations of the window-key type — but the Outcome line should have said "zero *window-key* policy-site string checks" rather than implying a bare grep would show nothing else.

Advisory follow-up (post-closeout triage): added `testPreP1604FixtureLoadsAllThreeWindowKeyShapes` and `testSaveAfterPreP1604FixtureLoadPreservesLegacyEntriesByteForByte` to `AppStateTests.swift` — hand-written raw JSON standing in for a real pre-P16.04 `app-state.json` (bare origin, `origin:sessionID`, and literal `"combined"` keys in `floating_pet_positions`/`floating_pet_hidden`), loaded and re-saved through today's `AppStateStore`/`WindowKey` code, confirming byte-compatible round-trip. Closes the "no end-to-end fixture test" advisory gap; full suite green (1049/1049).
