...
| R1.3 | Rename… item | Present whenever `currentSessionLabel != nil` OR `hasActiveSessionBadge` (session-keyed, plain-origin, and combined windows all offer it; a live session whose upstream platform never auto-titled the thread still offers Rename, since it's the only affordance able to give it a label at all) | Same gate | Same gate | intentional | intentional |
