...
**v3_preview (Phase 20)** makes the prompt timer and Settings > Sessions truthful across window hide/show, idle TTL dismiss, fold changes, and app relaunch by stamping four sticky timestamps (`prompt_started_at`, `session_started_at`, `errored_since`, `turn_ended_at`) directly onto each session's on-disk state — no more elapsed time resetting just because a panel wasn't the one holding the in-memory timer. Settings > Sessions rows also gain a relative **Started …** line when that data exists. **Dogfooders: the app and your hook install must be refreshed together for this release** — the state-file schema bumped to v10, and an old hook + new app (or vice versa) will not show the new timer/Started behavior correctly. There is intentionally no in-app "hooks outdated" nudge yet; re-run **Settings → General → Install / Update hooks** for every platform you use after updating the app.
