...
SOURCE_LAYOUTS = {
    # 4x2 is the ONLY layout agents generate (8 frames, no wasted/empty cell).
    # 3x3 is retained solely as the internal canonical geometry the slicer consumes;
    # normalize_generated_sheet.py converts the generated 4x2 into it.
    "4x2": {
        "cols": 4,
        "rows": 2,
        "width": 768,
        "height": 416,
        "note": "The generation layout: 8 populated 192x208 cells in a 4x2 sheet, no empty cell.",
    },
    "3x3": {
        "cols": 3,
        "rows": 3,
        "width": 576,
        "height": 624,
        "note": "Internal canonical geometry only (slicer input); not generated by agents.",
    },
}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Generation layout for sheet-first row candidates. Always 4x2 (4 cols x 2 rows, 8 frames, "
             "no empty cell); the pipeline normalizes it to the internal 3x3 canonical before slicing."
    )
