...
/// pruned session lands at the identical disk end-state as automatic TTL
/// expiry plus the automatic orphan sweeps.
///
...
enum SessionPruner {
	/// Deletes the slice and removes the label and cached-title keys for
	/// `windowKey`. Each step is independently best-effort (a missing slice
	/// or an unset label/title is a safe no-op), so a partial prior state
	/// never blocks the remaining steps.
